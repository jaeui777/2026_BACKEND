...
SELECT 제품명, 카테고리명 FROM product p INNER JOIN pcategory c ON p.카테고리번호_fk = pc.카테고리번호_pk;

-- [문제 2] '노트북' 카테고리에 속하는 모든 제품의 제품명과 제품가격을 조회하세요. 
SELECT * FROM pcategory pc WHERE 카테고리명 = '노트북'; -- 카테고리에 '노트북'

SELECT * FROM pcategory pc INNER JOIN product p ON pc.
...
SELECT p.제품명, c.카테고리명, s.재고수량  FROM product p  INNER JOIN pcategory c ON p.카테고리번호_fk = c.카테고리번호_pk  INNER JOIN stock s ON p.제품번호_pk = s.제품번호_fk;
