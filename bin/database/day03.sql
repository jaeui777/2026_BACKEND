...
select name from test; -- 테이블내 '이름' 속성명의 레코드 조회

select name, count from test; -- '이름' '개수' 속성명의 레코드 조회

select * from test where name = "유재석"; -- 이름 속성명의 값이 유재석이면 조회

select * from test where count >= 5; -- 개수 속성명의 값이 5 이상이면 조회

# [3] update : update 테이블명 set 속성명 = 새로운값 , 속성명 = 새로운 값 where 조건

update test set count = 10; -- 조건이 없으므로 테이블내 '개수'속성들의 값 *모두*

# [4] delete : delete from 테이블명 where 조건
-- de;ete from test; -- 조건이 없으므로 테이블내 모든 레코드 제거

delete from test where name = "유재석";

delete from test where no = 2; --번호 속성값이 2이면 삭제
--(DML)delete : 테이블내 레코드 삭제 [vs] (DDL)truncate talbe : 테이블내 레코드 삭제 [vs] drop talbe : 테이블 자체 삭제
...
select mid as 회원아이디 from member; -- 조회 결과 mid 속성명을 회원아이디로 별칭

select mid as 회원아이디 from member 회원테이블; --SQL내 속성명/테이블명 별칭
...
-- 문자열 패턴 : 속성명 like "문자패턴" , %: 모든 문자 대응 , : 개사만큼문자대응
