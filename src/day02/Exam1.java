...
public class Exam1 {
        public static void main(String[] args) {
            ...
            // == 다형성(상속) ==
            // [1] 자동 타입 변환
            byte bytevalue = 100;   // byte타입에 bytevalue 변수명으로 100 리터럴 대입했다.
            ...
            long result4 = i1 + l1 ; // int + long => long (더 큰 타입으로 결과 반환)
            float result5 = i1 + f1; // int + float => float
            double result6 = i1 + d1; // int + double => double
            // [2] 강제(명시적) 타입 변환 : byte <- short <- int <- long <- float <- double . 큰것 -> 작은것 , *자료 손실 발생할 수도 있다
            double dvalue = 3.14;
            // float fvalue = dvalue ; // [불가능] 
            float fvalue = (float)dvalue;   // 자료 앞에 (변환할타입명) 명시 하여금 강제로 타입 변환 가능 
            // long lvalue = fvalue; // [불가능]
