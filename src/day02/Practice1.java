...
public class Practice1 { // class 시작
...
/* 문제6: 아래와 같이 이스케이프 문자를 이용하여 콘솔에 출력하는 코드를 작성하시오.
 ...
 ||_/=\\__|
 */
System.out.println("|\\_/|"); // \를 두번 넣어야 \ 하나가 출력
...
// 1) scanner 객체 생성 2) nextxxx()함수
        Scanner scan = new Scanner(System.in);
        System.out.println("이름을 입력하세요:"); String 이름 =  scan.next( ); // 2) 적절한 nextxxx()을 선택
        System.out.println("나이를 입력하세요:"); int 나이 =  scan.nextInt( ); // 2) nextxxx() 반환되는 타입에 따라 변수의 타입 선정
...
내용: 정말입니다. 다들 동의하시죠?
*/
// nextLine 주의할점 : 만약에 nextLine 앞에 다른 nextXX()가 존재한 경우에는 사이에 의미없는 nextLine() 추가
...
// 10. nextchar()는 제공하지 않는다. next().charAt(인덱스) : 해당 인덱스의 문자 1개 추출함수 , *문자열은 배열이다.*
System.out.print("성별을 입력하세요(남/여): "); char 성별 = scan.next().charAt(0);
System.out.println("입력하신 성별은 %s 입니다. \n , 성별");
