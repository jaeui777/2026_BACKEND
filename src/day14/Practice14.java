...
public class Practice {
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            boolean carfind = false; // 차량 찾았는지 확인
            // 차량 하나씩 확인 반복문(?)
                for (String car : cars) {
                    String[ ] 위치 = car.split(",");

                    // 입력한 차량번호와 비교
                    if (위치[1].equals(carNumber)) {
                    System.out.println("주차 위치 : " + 위치[0]);
                    carfind = true;
            }
            else { System.out.println("미등록 차량");}
             } 
             }
            ...
            boolean 중복 = false;   // 위치 중복여부 확인
             for (String car : cars) {
                    String[ ] 위치 = car.split(",");

                    // 입력한 차량번호와 비교
                    if (위치[0].equals(location)) { System.out.println( "위치 번호에는 중복 주차할 수 없습니다.");
            } else { System.out.println( "입차 완료");
